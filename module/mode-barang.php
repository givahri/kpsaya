...
function insert($data){
    ...
    if (mysqli_num_rows($cekBarcode)){
        echo '<script> 
                     alert("Kode barcode sudah ada, barang gagal di tambalkan")
            </script>';
        return false;
    }
    ...
    $sqlBrg  = "INSERT INTO tbl_barang VALUE ('$id', '$barcode', '$name', $harga_beli, 0, $harga_jual, '$satuan', $stockmin, '$gambar')";
...
function update($data) {
    ...
    $harga_beli = mysqli_real_escape_string($koneksi, $data['harga_beli']);
    $harga_jual = mysqli_real_escape_string($koneksi, $data['harga_jual']);
    $stokmin   = mysqli_real_escape_string($koneksi, $data['stock_minimal']);
    ...
    if ($barcode !== $curBarcode) {
            ...
            if (mysqli_num_rows($cekBarcode)){
                echo '<script> 
                             alert("Kode barcode sudah ada, barang gagal di perbarui")
                    </script>';
                return false;
            }
    }
